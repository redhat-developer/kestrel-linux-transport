...
extern "C"
{
    ...
    PosixResult TmdsKL_Connect(intptr_t socket, PalSocketAddress* palSocketAddress, int32_t palEndPointLen);
    PosixResult TmdsKL_Bind(intptr_t socket, PalSocketAddress* palSocketAddress, int32_t palEndPointLen);
    PosixResult TmdsKL_GetAvailableBytes(intptr_t socket);
...
PosixResult TmdsKL_Listen(intptr_t socket, int32_t backlog)
{
    int fd = ToFileDescriptor(socket);
...
PosixResult TmdsKL_Accept(intptr_t socket, PalSocketAddress* palSocketAddress, int32_t palEndPointLen, int blocking, intptr_t* acceptedSocket)
{
